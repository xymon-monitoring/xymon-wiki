# Xymon developer guide (legacy)

*Historical reference from the Hobbit / SourceForge-SVN era. The content is obsolete — Xymon development has since moved to GitHub — and is preserved here for historical interest.*
...
#### References
  ...
  - [Getting started with
    SVN](http://artis.imag.fr/~Xavier.Decoret/resources/svn/index.html)
  - [GCC SVN tutorial](http://gcc.gnu.org/wiki/SvnSetup)

#### SVN for Xymon Tutorials

Followings are some examples using svn to interact with hobbit on
sourceforge.net.
...
```
  ...
  - As one of [hobbit
    developers](http://sourceforge.net/project/memberlist.php?group_id=128058),
...
Version 4.5:
  ...
  - Support for client authentication via SSL certificates (so you
    cannot spoof a client message).

Version 4.6:

  - Revised web interface - eliminate the need for webserver-based
    authentication, and provide more fine-grained authorization for the
    various Xymon components.
...
### Wish Lists
  ...
  - Provide encrypted communication pipe between hobbit server and
    hobbit client.
  - hobbit messages compression.
  - ipv6
  - 4.2.x maintenance release.
  - Rewrite of the alerts module to work properly. ;) It's quite flawed
    at the moment.

## Xymon volunteers Roles and Responsibility

This is a list of people offer their time and skill to Xymon community
in following areas of R\&R.

1.  Project Admin
    1.  Has right as project admin on sourceforge.
    2.  Assign tickets to different persons listed below.
    3.  [Grant new developer privilege on
        sourceforge](http://sourceforge.net/project/memberlist.php?group_id=128058).
    4.  Get people to work as a team with limited and un-paid resources.
2.  Development
    1.  Understand Xymon source tree.
    2.  Do source code documentation.
    3.  Work on ticket from issue tracker.
    4.  Apply patches submitted from the Xymon community.
    5.  Implement new requested features
3.  Documentation
    1.  Keep troff files updated with Xymon features.
        1.  maintain troff2html process.
    2.  Create a publish-grade manuals and process for Xymon using
        Latex.
        1.  use latex2troff tool to generate Xymon troff manpage
            sources.
        2.  Automate the document generation in "make" command process.
4.  End User Support
    1.  Answer questions raised in xymon user email list.
    2.  Maintain [Xymon
        FAQ](http://en.wikibooks.org/wiki/System_Monitoring_with_Xymon/Other_Docs/FAQ).
    3.  Test out new Xymon features or verify a bug.
5.  Module Development
    1.  Write new Xymon client external modules.
    2.  Write Xymon Server side external modules.
    3.  Maintain orphan Xymon modules.
    4.  Maintain Xymon module development guides.

| Name                                   | Project Admin | Development | Documentation | End User Support | Module Development |
| -------------------------------------- | ------------- | ----------- | ------------- | ---------------- | ------------------ |
| Henrik Størner                         |               | Yes         |               |                  |                    |
| T.J. Yang                              |               | Yes         | Yes           |                  | Yes                |
| Jerald Sheets                          |               |             | Yes           |                  |                    |
| Josh Luthman                           |               |             |               |                  |                    |
| Malcolm Hunter                         |               |             | Yes           | Yes              |                    |
| Japheth J.C. Cleaver                   |               |             |               |                  |                    |
| Steve Holmes                           |               |             | Y             | Y                | ?                  |
| Damien Martins (aka doctor madness)    |               | N           | Y             | Y                | N                  |
| dubby                                  |               |             |               | Yes              |                    |
| Flemming                               |               |             |               | Yes              |                    |
| [Christoph Berg](http://www.df7cb.de/) |               | Yes         |               |                  | Yes                |
| Vadim Pushkin                          |               |             |               | Yes              |                    |
| Martin Ward                            |               | Yes         |               |                  |                    |
| Buchan Milne                           |               | Yes         |               |                  |                    |
| Rich Smrcina                           |               | Yes         |               |                  |                    |
| Kenneth Langford                       |               | Yes         |               |                  |                    |
| Padraig Lennon                         |               |             |               |                  |                    |
| David Baldwin                          |               | Yes         |               |                  |                    |
| Ralph Mitchell                         |               |             | Yes           | Yes              |                    |
| Dummy Template Replace Here            | Admin Y/N     | Dev Y/N     | Doc Y/N       | User Support Y/N | Module devel Y/N   |
| Olivier AUDRY                          |               | Yes         |               |                  | Yes                |
| Neil Franken                           |               | Yes         |               | Yes              |                    |
| Peter Toft                             |               |             |               |                  |                    |
...
### [Developers with write access](http://sourceforge.net/project/memberlist.php?group_id=128058)

  - Henrik Stoerner: <http://www.hswn.dk>
      - Make Big Brother system monitor to be open again.
      - Rewrite Big Brother shell scrip part into C code to speed up the
        performance.
      - [Amazon Wish
        List](http://www.amazon.co.uk/gp/registry/registry.html/ref=w_h_em-si-html_viewall/026-3569182-7564469?id=2EMUDL12T4WA4)

<!-- end list -->

  - [Martin
    Ward](http://sourceforge.net/people/viewprofile.php?user_id=2101940)

<!-- end list -->

  - Rich Smrcina
  - Buchan Milne
  - [T.J. Yang](http://en.wikibooks.org/wiki/User:Tjyang)
      - hobbit wiki book project.
      - [Autoconfiscate
        Hobbit](http://en.wikibooks.org/wiki/System_Monitoring_with_Hobbit/Developer_Guide#Autoconfiscate_Hobbit).
      - Hobbit documentation project.
          - Will use latex to create hobbit manual.
      - Digitation of hobbit software build,package build and deployment
        process.
...
<!-- end list -->
  ...
  - [TWW Inc.](http://www.thewrittenword.com)
...
This section describe the conversion of hobbit source tree into autotool
control.
...
### Learn autotool
  ...
  - [the autotools
    tutorial](http://www.lrde.epita.fr/~adl/autotools.html) and read the
    [pdf slides](http://www.lrde.epita.fr/~adl/dl/autotools.pdf).
  - hand on testing on hello world example.
  - [example of using
    autoscan](http://mij.oltrelinux.com/devel/autoconf-automake/).
...
<!-- end list -->
  ...
  - download the [latest hobbit source tar
    ball](http://www.hswn.dk/beta/hobbit-snapshot.tar.gz).
  - run autoscan to get configure.scan and then modify it as
    configure.ac.
  - convert lib/Makefile demotool/Makefile ... etc into Makfile.am.
  - Fine tune configure.ac to take configuration options as configure
    options.
  - Support Linux and Solaris OS setting in configure.ac.

### References

  - [Great Autotool tutorials and links started
    here](http://www.lrde.epita.fr/~adl/autotools.html)
  - [Another
    tutorial](http://mij.oltrelinux.com/devel/autoconf-automake/)
  - [Autotool for
    Beginners](http://vindaci.members.sonic.net/cbreak/projects/autotools/)

## Xymon Documentation

1.  Current
    1.  Henrik Stroner use troff to generate manual pages so that we
        have the RTFM type of documentation.
    2.  Also troff2html is used to create html page for web browser
        viewing.
2.  Enhancements Proposals.
    1.  Use latex+dia+make+wiki to create publishing grade docs
    2.  Use Table of Conetent, index, references and Figures to describe
        Xymon related topic.
    3.  Generate Xymon User Guide.
    4.  Generate Xymon Developer Guide
    5.  Generate Xymon System Admin Guide.
    6.  Generate Xymon Slide for presentation purpose.
    7.  Use Wiki documentation content incubation.
    8.  Generate html.RTF and pdf for different purpose.
3.  Request Development Team to use doxygen syntax for source code
    documentation.
...
#### shell scripts
  ...
  - test script in build directory of hobbit-4.0.3 to make sure depended
    software are in build machine.
...
[An example of putting dtrace probe int an
application](http://blogs.sun.com/barts/entry/putting_user_defined_dtrace_probe).
...
### Hobbit Client for OpenSolaris IPS format
  ...
  - Features of This hobbit client
      - This hobbit client will use a local account and group to avoid
        NIS and NFS dependency.
      - It will use SMF to start/top hobbit client.
      - It will use sudo or Solaris 10's ACL to achieve reading
        restricted information off OpenSolaris.
      - hobbit-client-serverconf is simple version of hobbit client
        without local messages processing.
      - hobbit-client-local will do some processing filtering, it need
        more depended software.
...
<!-- end list -->
  ...
  - Installed Sun and SUNW gcc-3.4.3 compilers.
      - if not installed run "pkg install sunstudioexpress" and "pkg
        install SUNWgcc".
...
## How to participate the hobbit development
  ...
  - create an account on en.wikibooks.org to participate in hobbit
    documentation effort.
  - learn from <http://en.wikibooks.org/wiki/CPAM_with_TWW> to
    participate in hobbit automated cross-platform software packaging
    effort.
...
[BB Win](http://bbwin.sourceforge.net/)
...
[System Monitoring with Hobbit/HOWTO/Custom Monitoring Scripts for
Hobbit](System_Monitoring_with_Hobbit/HOWTO/Custom_Monitoring_Scripts_for_Hobbit "wikilink")
...
### Why use CPAM/TWW tool ?
  ...
  - Pros
      - Hobbit server and client need to be cross-platform thus the need
        of CPAM/TWW.
      - Like Emacs editor, it can will be run everywhere without
        license. TWW tools is GPL opensource so it can be ported to any
        OS.
      - A structural and automated approach to deal with packages
...
1.  bbwin (Xymon client) is implemented in c++. It requires opensource
    software like boost c++ libary and tinyXML.
2.  The packaging tool it used is Microsoft's opensource packaging tool
    - [WiX](http://sourceforge.net/projects/wix/)to do the Win32
    packaging.
3.  The detals building instruction is
    [build.txt](http://bbwin.svn.sourceforge.net/viewvc/bbwin/trunk/Build.txt?revision=21&view=markup)
    on bbwin project site.
4.  You have two ways to build BBWin.msi,
    1.  Use Microsoft's make: nmake
    2.  Use "Build Soultion" in VS2008.

Following is a procedure to create a Windows XP development box so you
can create your own BBWin.msi.

## Prepare your Xymon win32 build environment

1.  Install
    [hhc.exe](http://msdn.microsoft.com/en-us/library/ms669985),it is
    used to compile html doc into windows chm help file.
    1.  add "C:\\Program Files\\HTML Help Workshop" into PATH so hhc.exe
        is accessible.
2.  Install Visual Studio 2008 Standard or Professional Edition.
3.  Make sure your cmd.exe has following PATH name addition. echo %PATH%
    %INCLUDE% %LIB% in your DOS prompt.
    1.  PATH
    2.  "LIB=C:\\Program Files\\Microsoft Platform
        SDK\\Lib\\.;C:\\Program Files\\Microsoft Visual Studio
        8\\VC\\LIB;"
    3.  INCLUDE
...
1.  Install a svn client for your windows machine.
    1.  [tortoise svn](http://tortoisesvn.net/downloads)
2.  [Windows Server 2003 SP1 Platform SDK Web
    Install](http://www.microsoft.com/downloads/details.aspx?FamilyId=A55B6B43-E24F-4EA3-A93E-40C0EC4F68E5&displaylang=en):
    It is use to have the complete sdk to develop programs on Microsoft
    OS. Remeber to install all options (include 64 bit and 32 bit) to
    avoid "atlthunk.lib not found" issue.
    1.  [install visual service
        pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=7B0B0339-613A-46E6-AB4D-080D4D4A8C4E&displaylang=en):
    2.  [MSDN blog about VS2008 and
        SDK](http://blogs.msdn.com/windowssdk/archive/2008/02/22/using-visual-c-2008-express-with-the-windows-sdk-short-version.aspx)
3.  [WiX opensource packaging tool from
    Microsoft](http://wix.sourceforge.net/releases/): this is use to
    turn the compiled binary into MS MSI package format.
4.  [Boost C++ library](http://www.boost.org/):These powerful C++
    libraries are used for some agents and will be certainly used in the
    BBWin core service in the future.
    1.  [Download the pre-compiled 1.35 one for
        VS2008](http://www.boostpro.com/download/) and install it under
        C:\\boost directory.
5.  [TinyXML](http://www.grinninglizard.com/tinyxml/): BBWin used the
    excellent TinyXML library for the configuration loading XML. You do
    not need to download and install it. TinyXML sources are included in
    BBWin sources.
6.  Prepare your source code editors:
    1.  [notepade++](http://notepad-plus.sourceforge.net):
    2.  [XEmacs for win32](http://www.xemacs.org/Download/win32/):

<!-- end list -->

1.  checkout bbwin src.
    1.  Using command "svn co
        <https://bbwin.svn.sourceforge.net/svnroot/bbwin> bbwin"
2.  Change the Makefile code for C:\\boost installation.
    1.  Modify Makefile to point INCLUDE to C:\\boost and LIB to include
        C:\\boost\\lib\\\*.lib
    2.  Modify C/C++ include c:\\boost and Linker to include
        c:\\boost\\lib\\\*.
3.  Optional software installation
    1.  install cygwin to have a strong unix enviroment on your windows
        box. So you can run command like following to clean up \*.obj
        file that current makefile can't do.
    2.  Clean up Release directory by Deleting Setup/\*.msi
...
BBWin.msi creation process is automated in
"C:\\BBWin\\trunk\\Setup\\Makefile". This makefile will be called from
"C:\\BBWin\\trunk\\Makefile"
...
<!-- end list -->
  ...
  - make sure you have following directory and file, if not then you
    need to use VS non-Express version.
...
<!-- end list -->
  ...
  - [Possible
    fix](http://www.gamedev.net/community/forums/topic.asp?topic_id=484760)

##### Temporary Fix for following

  - Goal is to get rid of " -D\_MT -MTd" when using cl MS compiler.
  - Run this command in cygwin shell on your codebase, "find . -type f
    -name Makefile -exec perl -pi -e 's\!cvarsmt\!conlibsdll\!' {} \\;"
    , to replace cvarsmt with conlibsdll. I don't know the exact impact
    of not building code with multihreading library.
...
##### \* LINK : warning LNK4098: defaultlib 'LIBCMT' conflicts with use of other libs; use /NODEFAULTLIB:library
  ...
  - [Possible fix with similar
    issue](http://www.omnetpp.org/listarchive/msg07806.php).
  - [vc6 to vc8](http://www.omnetpp.org/listarchive/msg07798.php).
...
#### How to use hobbit-4.0.3.sb build source ?
  ...
  - hobbit-4.0.3.sb is an XML file contain the detail exact steps to
    build hobbit-4.03. into binary code for different OS. It need to be
    run against "sb" tool.
  - TBA.
...
#### SQLite3
  ...
  - Just like another rrd file that can be rsynced over to your standby
    xymon server.
...
Current Xymon(Hobbit) is coded in C and using CGI and html code to
compose a simple web interface. Recently, we have a few javascript
framework choices to upgrade xymon server with RIA web interface.

#### Using qwikioffice for Xymon Dashboard

  - [qwikioffice Demo
    Site](http://www.qwikioffice.com/desktop-demo/login.html)

#### Using Tomatocart's admin tool for Xymon administration tool

  - [Xymon Admin
    Interface](http://xymon.dlinkddns.com/to/admin/index.php?language=en_US)

#### Replace Xymon C/CGI with C/EXTJS

  - [JSON in C](http://oss.metaparadigm.com/json-c/)
...
### GCC Compilation Warnings
  ...
  - pointer targets in assignment differ in signedness
      - [Why use unsigned char
        ?](http://www.velocityreviews.com/forums/t276758-why-use-unsigned-char-ever.html)
      - [char-vs-unsigned-char](http://bytes.com/topic/c/answers/473994-char-vs-unsigned-char)
  - "/usr/include/sys/socket.h:214: note: expected socklen\_t \*
    \_\_restrict\_\_ but argument is of type int \*"
...
3.  Provide realtime web GUI update from clients using AJAX.
    1.  Waiting for 1+ minutes to see the changes from web page is
        barely acceptable but hope to avoid.
...
I found following combination of existing opensource software can make
up a new GUI for Xymon. End user can still see old cgi+html GUI.

1.  Use extjs for new GUI and miframe.js to contain old tigra menu GUI
    in a frame.
2.  See [Xymon Desktop
    Prototype](http://xymon.dlinkddns.com:8083/hobbit/ext3/examples/desktop/).

## Using command line tool to update this wiki book

### Installation of mediawiki client in perl

  - [Mediawik command line tool](http://search.cpan.org/~markj/)
  - A makefile script to automate the hobbit documentation effort.

---

*Source: ["System Monitoring with Xymon / Developer Guide"](https://en.wikibooks.org/wiki/System_Monitoring_with_Xymon/Developer_Guide) on Wikibooks, used under [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/).*
